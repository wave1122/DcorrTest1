...
int main(void) {
	...
	/**************************************************** IMPORT THE FIRST DATASET *************************************************************/
     ifstream readX1; //returns
	readX1.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Indices\\returns.csv", ios::in);
    if (!readX1)
        return(EXIT_FAILURE);
    vector<vector<string>* > matX1;
    loadCSV(readX1, matX1);
    int T = matX1.size(); //Rows
    int N = (*matX1[1]).size(); //Columns
    Matrix X(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            X(t,i) = hex2d((*matX1[t-1])[i-1]);
        }
        cout << X(t,1) <<  " , ";
    }
    cout << " \n " << endl;
    for (vector<vector<string>*>::iterator p1 = matX1.begin( ); p1 != matX1.end( ); ++p1)
    {
      delete *p1;
    }
    readX1.close();

    ifstream readX2; //conditional means
	readX2.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Indices\\cmeans.csv", ios::in);
    if (!readX2)
        return(EXIT_FAILURE);
    vector<vector<string>* > matX2;
    loadCSV(readX2, matX2);
    T = matX2.size(); //Rows
    N = (*matX2[1]).size(); //Columns
    Matrix cmean_X(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            cmean_X(t,i) = hex2d((*matX2[t-1])[i-1]);
        }
        cout << cmean_X(t, 1) << " , ";
    }
    cout << " \n" << endl;
    for (vector<vector<string>*>::iterator p1 = matX2.begin( ); p1 != matX2.end( ); ++p1)
    {
      delete *p1;
    }
    readX2.close();

    ifstream readX3; //the sqrts of conditional variances and covariances
	readX3.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Indices\\ccovs.csv", ios::in);
    if (!readX3)
        return(EXIT_FAILURE);
     vector<vector<string>* > matX3;
    loadCSV(readX3, matX3);
    T = matX3.size(); //Rows
    N = (*matX3[1]).size(); //Columns
    Matrix csigma_X(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            csigma_X(t,i) = hex2d((*matX3[t-1])[i-1]);
        }
        cout << csigma_X(t,1) << " , ";
    }
    cout << " \n" << endl;
    for (vector<vector<string>*>::iterator p1 = matX3.begin( ); p1 != matX3.end( ); ++p1)
    {
      delete *p1;
    }
    readX3.close();

    ifstream readX4; //MGARCH residuals
	readX4.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Indices\\residuals.csv", ios::in);
    if (!readX4)
        return(EXIT_FAILURE);
    vector<vector<string>* > matX4;
    loadCSV(readX4, matX4);
    T = matX4.size(); //Rows
    N = (*matX4[1]).size(); //Columns
    Matrix resid_X(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            resid_X(t,i) = hex2d((*matX4[t-1])[i-1]);
        }
        cout << resid_X(t,1) << "  , ";
    }
    cout << " \n" << endl;
    for (vector<vector<string>*>::iterator p1 = matX4.begin( ); p1 != matX4.end( ); ++p1)
    {
      delete *p1;
    }
    readX4.close();
    ...
    ifstream readY1; //returns
	readY1.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Features\\returns.csv", ios::in);
    if (!readY1)
        return(EXIT_FAILURE);
    vector<vector<string>* > matY1;
    loadCSV(readY1, matY1);
    T = matY1.size(); //Rows
    N = (*matY1[1]).size(); //Columns
    Matrix Y(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            Y(t,i) = hex2d((*matY1[t-1])[i-1]);
        }
        cout << Y(t,1) <<  " , ";
    }
    cout << "\n" << endl;
    for (vector<vector<string>*>::iterator p1 = matY1.begin( ); p1 != matY1.end( ); ++p1)
    {
      delete *p1;
    }
    readY1.close();

    ifstream readY2; //conditional means
	readY2.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Features\\cmeans.csv", ios::in);
    if (!readY2)
        return(EXIT_FAILURE);
    vector<vector<string>* > matY2;
    loadCSV(readY2, matY2);
    T = matY2.size(); //Rows
    N = (*matY2[1]).size(); //Columns
    Matrix cmean_Y(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            cmean_Y(t,i) = hex2d((*matY2[t-1])[i-1]);
        }
        cout << cmean_Y(t,1) <<  " , ";
    }
    cout << " \n " << endl;
    for (vector<vector<string>*>::iterator p1 = matY2.begin( ); p1 != matY2.end( ); ++p1)
    {
      delete *p1;
    }
    readY2.close();

    ifstream readY3; //the sqrts of conditional variances and covariances
	readY3.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Features\\ccovs.csv", ios::in);
    if (!readY3)
        return(EXIT_FAILURE);
     vector<vector<string>* > matY3;
    loadCSV(readY3, matY3);
    T = matY3.size(); //Rows
    N = (*matY3[1]).size(); //Columns
    Matrix csigma_Y(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            csigma_Y(t,i) = hex2d((*matY3[t-1])[i-1]);
        }
        cout << csigma_Y(t,1) <<  " , ";
    }
    cout << " \n " << endl;
    for (vector<vector<string>*>::iterator p1 = matY3.begin( ); p1 != matY3.end( ); ++p1)
    {
      delete *p1;
    }
    readY3.close();

    ifstream readY4; //MGARCH residuals
	readY4.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\data\\GT\\Features\\residuals.csv", ios::in);
    if (!readY4)
        return(EXIT_FAILURE);
    vector<vector<string>* > matY4;
    loadCSV(readY4, matY4);
    T = matY4.size(); //Rows
    N = (*matY4[1]).size(); //Columns
    Matrix resid_Y(T,N);
    for (int t = 1; t <= T; t++)
    {
        for (int i = 1; i <= N; i++)
        {
            resid_Y(t,i) = hex2d((*matY4[t-1])[i-1]);
        }
        cout << resid_Y(t,1) <<  " , ";
    }
    cout << " \n " << endl;
    for (vector<vector<string>*>::iterator p1 = matY4.begin( ); p1 != matY4.end( ); ++p1)
    {
      delete *p1;
    }
    readY4.close();
 /************************************************************************************************************************************************/
    ...
    int lag_smooth = 20; //set a kernel bandwidth: 5, 10, 15, 20, 25, 30, 35, 40, 45
	...
	auto cstat = 0.;
	Matrix cstat_bootstrap(num_B, 1);
	auto pvalue = NGDist_corr::calcul_Pvalue <bartlett_kernel> (cstat, cstat_bootstrap, X, Y, cmean_X, cmean_Y, csigma_X, csigma_Y, resid_X, resid_Y, lag_smooth, xi_x,  xi_y, expn);

    ofstream  output; //open an output stream
	output.open ("E:\\Copy\\SCRIPTS\\Exogeneity\\Application\\output\\GT\\results.txt", ios::out);
