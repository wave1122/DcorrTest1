...
int main(void) {
    ...
    //*************************************************************using the Bartlett kernel****************************************************************************/
     output_filename = "./Results/NonGaussian/SN/Others/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                     + "_rho=" + std::to_string(rho) + "_bartlett_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/SN/Others/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
	                                + "_rho=" + std::to_string(rho) + "_bartlett_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/SN/Others/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                     + "_rho=" + std::to_string(rho) + "_bartlett_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
	...
	//calculate empirical critical values and sizes
     Dep_tests::cValue <bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_SN>, NL_Dgp::est_BL> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, delta, seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << asymp_CV_Hong(1) << " , "
                 << asymp_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: 5%- and 10%- sizes for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << REJF_Hong(1) << " , " << REJF_Hong(2) << ")" << endl;
	output << "Hong's (2001) ecf-based test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_ECFTest(1) << " , "
                 << empir_CV_ECFTest(2) << ")" << endl;
     output << "Hong's (2001) ecf-based test: 5%- and 10%- sizes for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << REJF_ECFTest(1) << " , " << REJF_ECFTest(2) << ")" << endl;
	output << "Haugh's (1976) Portmanteau test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_Haugh(1) << " , "
                 << empir_CV_Haugh(2) << ")" << endl;
     output << "Haugh's (1976) Portmanteau test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << asymp_CV_Haugh(1) << " , "
                 << asymp_CV_Haugh(2) << ")" << endl;
     output << "Haugh's (1976) Portmanteau test: 5%- and 10%- sizes for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << REJF_Haugh(1) << " , " << REJF_Haugh(2)
                 << ")" << endl;
	//calculate rejection rates
	choose_alt = 1; //correlated errors
     Dep_tests::power_f<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_SN>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                ...
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, delta, rho, choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    //***********************************************************using the Daniell kernel*************************************************************************
     output_filename = "./Results/NonGaussian/SN/Others/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                    + "_rho=" + std::to_string(rho) + "_daniell_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/SN/Others/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
	                               + "_rho=" + std::to_string(rho) + "_daniell_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/SN/Others/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                    + "_rho=" + std::to_string(rho) + "_daniell_kernel_univariate.txt";
     ...
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_SN>, NL_Dgp::est_BL> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, delta, seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;
     Dep_tests::power_f<daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_SN>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, delta, rho, choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    //******************************************************************using the QS kernel***********************************************************************
     output_filename = "./Results/NonGaussian/SN/Others/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                     + "_rho=" + std::to_string(rho) + "_QS_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/SN/Others/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
	                                + "_rho=" + std::to_string(rho) + "_QS_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/SN/Others/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                     + "_rho=" + std::to_string(rho) + "_QS_kernel_univariate.txt";
     ...
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_SN>, NL_Dgp::est_BL> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, delta, seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;
     Dep_tests::power_f<QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_SN>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, delta, rho, choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    //*************************************************************using the Bartlett kernel****************************************************************************/
     output_filename = "./Results/NonGaussian/SN/Others/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                      + "_rho=" + std::to_string(rho) + "_bartlett_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/SN/Others/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                      + "_rho=" + std::to_string(rho) + "_bartlett_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/SN/Others/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
                                      + "_rho=" + std::to_string(rho) + "_bartlett_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_SN>, NL_Dgp::est_TAR> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, delta, seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1; //correlated errors
     Dep_tests::power_f<bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_SN>, NL_Dgp::est_TAR> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, delta, rho, choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
	...
	size_filename = "./Results/NonGaussian/SN/Others/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
     ...
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_SN>, NL_Dgp::est_TAR> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, delta, seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;
     Dep_tests::power_f<daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_SN>, NL_Dgp::est_TAR> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, delta, rho, choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
	...
	size_filename = "./Results/NonGaussian/SN/Others/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_delta=" + std::to_string(delta)
     ...
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_SN>, NL_Dgp::est_TAR> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, delta, seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " and delta= " << delta <<  " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;
     Dep_tests::power_f<QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_SN>, NL_Dgp::est_TAR> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, delta, rho, choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    /************************************************************************************************************************************************/
     ...
     delta(2) = 0.9;
     delta(3) = -0.1;
     double rho12 = -0.5, rho13 = 0.2;
     unsigned long seed = 856764325;
     string output_filename, size_filename, power_filename;
     ofstream output, size_out, pwr_out;
     bandW = 5;
     output << "Lag smooth length = " << bandW << endl;
     output_filename = "./Results/NonGaussian/SN/Others/file_out_BL_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_delta=" + std::to_string(delta(1)) + "_"
                                    + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_HRTest.txt";
	size_filename = "./Results/NonGaussian/SN/Others/size_out_BL_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_delta=" + std::to_string(delta(1)) + "_"
                                    + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_HRTest.txt";
     power_filename = "./Results/NonGaussian/SN/Others/power_out_BL_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_delta=" + std::to_string(delta(1)) + "_"
                                    + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_HRTest.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     //calculate empirical critical values and sizes
	Matrix asymp_CV_ElHimdiRoy = Dep_tests::asymp_CV_ChiSq2 (bandW); //obtain 5%- and 10%- asymptotic critical values for the test
     Dep_tests::cValue <NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriSN>, NL_Dgp::est_BL> (REJF_ElHimdiRoy, empir_CV_ElHimdiRoy, alpha_X, alpha_Y, delta, asymp_CV_ElHimdiRoy,
     ...
     //calculate rejection rates
     choose_alt = 1; //errors are uncorrelated but dependent
     Dep_tests::power_f <NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriSN>, NL_Dgp::est_BL> (empir_REJF_ElHimdiRoy, asymp_REJF_ElHimdiRoy, alpha_X, alpha_Y, delta,
                                                                                                                                                           empir_CV_ElHimdiRoy, asymp_CV_ElHimdiRoy, rho12, rho13, choose_alt, number_sampl,
	...
	cout << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
     choose_alt = 2; //errors are corrrelated  (thus dependent)
     Dep_tests::power_f <NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriSN>, NL_Dgp::est_BL> (empir_REJF_ElHimdiRoy, asymp_REJF_ElHimdiRoy, alpha_X, alpha_Y, delta,
                                                                                                                                                           empir_CV_ElHimdiRoy, asymp_CV_ElHimdiRoy, rho12, rho13, choose_alt, number_sampl,
                                                                                                                                                           T, bandW, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    /************************************************************************************************************************************************/
     ...
     bandW = 5;
     output << "Lag smooth length = " << bandW << endl;
     output_filename = "./Results/NonGaussian/SN/Others/file_out_TAR_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_delta=" + std::to_string(delta(1)) + "_"
                                    + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_HRTest.txt";
	size_filename = "./Results/NonGaussian/SN/Others/size_out_TAR_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_delta=" + std::to_string(delta(1)) + "_"
                                    + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_HRTest.txt";
     power_filename = "./Results/NonGaussian/SN/Others/power_out_TAR_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_delta=" + std::to_string(delta(1)) + "_"
                                    + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_HRTest.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     //calculate empirical critical values and sizes
	Matrix asymp_CV_ElHimdiRoy = Dep_tests::asymp_CV_ChiSq2 (bandW); //obtain 5%- and 10%- asymptotic critical values for the test
     Dep_tests::cValue <NL_Dgp::gen_TAR<NL_Dgp::gen_TriSN>, NL_Dgp::est_TAR> (REJF_ElHimdiRoy, empir_CV_ElHimdiRoy, alpha_X, alpha_Y, delta, asymp_CV_ElHimdiRoy,
                                                                                                                                                          number_sampl, T, bandW, seed, size_out);
     output << "5%- and 10%- empirical critical values for bandW = " <<  bandW << ":  " << "(" << empir_CV_ElHimdiRoy(1) << " , " << empir_CV_ElHimdiRoy(2) << ")" << endl;
     output << "5%- and 10%- sizes for bandW = " <<  bandW << ":  " << "(" << REJF_ElHimdiRoy(1) << " , " << REJF_ElHimdiRoy(2) << ")" << endl;
     cout << "5%- and 10%- empirical critical values for bandW = " <<  bandW << ":  " << "(" << empir_CV_ElHimdiRoy(1) << " , " << empir_CV_ElHimdiRoy(2) << ")" << endl;
     ...
     //calculate rejection rates
     choose_alt = 1; //errors are uncorrelated but dependent
     Dep_tests::power_f <NL_Dgp::gen_TAR<NL_Dgp::gen_TriSN>, NL_Dgp::est_TAR> (empir_REJF_ElHimdiRoy, asymp_REJF_ElHimdiRoy, alpha_X, alpha_Y, delta,
                                                                                                                                                           empir_CV_ElHimdiRoy, asymp_CV_ElHimdiRoy, rho12, rho13, choose_alt, number_sampl,
                                                                                                                                                           T, bandW, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
     output << "choose_alt = " << choose_alt << endl;
