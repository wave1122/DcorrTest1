...
int main(void) {
   ...
   Matrix xi_x(num_B, T), xi_y(num_B, T); //generate two independent sequences of i.i.d. standard normal random variables
    ...
    /************************************************************ Using the Bartlett kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/MN/Bootstrap/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_delta=" + std::to_string(delta(1)) + "_"
                                   + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/MN/Bootstrap/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) +  "_delta=" + std::to_string(delta(1)) + "_"
                                   + std::to_string(delta(2)) + "_" + std::to_string(delta(3))  + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/MN/Bootstrap/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_delta=" + std::to_string(delta(1)) + "_"
                                   + std::to_string(delta(2)) + "_" + std::to_string(delta(3))  + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    output.open (output_filename.c_str(), ios::out);
    size_out.open (size_filename.c_str(), ios::out);
    pwr_out.open (power_filename.c_str(), ios::out);
...
NGPower::power_f <bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriMN>, NGReg::cmean_BL, NL_Dgp::est_BL> (bootstrap_REJF, number_sampl, TL, lag_smooth,
																											   alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s,
																											   eta2_t, eta2_s, xi_x, xi_y, delta, rho, 0., expn, 0,
																											   seed, size_out);
    ...
    NGPower::power_f <bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriMN>, NGReg::cmean_BL, NL_Dgp::est_BL> (bootstrap_REJF, number_sampl, TL, lag_smooth,
																																											   alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s,
																																											   eta2_t, eta2_s, xi_x, xi_y, delta, rho, 0., expn,
    ...
    //calculate 5%- and 10%- sizes of the bootstrap test
	NGPower::power_f <daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriMN>, NGReg::cmean_BL, NL_Dgp::est_BL> (bootstrap_REJF, number_sampl, TL, lag_smooth,
																																											   alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s,
																																											   eta2_t, eta2_s, xi_x, xi_y, delta, rho, 0., expn, 0,
																																											   seed, size_out);
    output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << bootstrap_REJF(1) << " , " << bootstrap_REJF(2) << ")" << endl;
    ...
    //calculate 5%- and 10%- sizes of the bootstrap test
	NGPower::power_f <QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriMN>, NGReg::cmean_BL, NL_Dgp::est_BL> (bootstrap_REJF, number_sampl, TL, lag_smooth,
																																											   alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s,
																																											   eta2_t, eta2_s, xi_x, xi_y, delta, rho, 0., expn, 0,
																																											   seed, size_out);
    output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << bootstrap_REJF(1) << " , " << bootstrap_REJF(2) << ")" << endl;
    ...
    /************************************************************************************************************************************************************/
     ...
     int T = 100;
     int N = 500;
	 int number_sampl = 500; //number of random samples drawn from the dgp
	 int num_B = 1000; //number of bootstrap samples
	 double expn = 1.5, rho = 0.5;
	 int choose_alt = 0;
     unsigned long seed = 856764325;
    ...
    /************************************************************ Using the Bartlett kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/MN/Bootstrap/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_delta=" + std::to_string(delta(1)) + "_"
                                   + std::to_string(delta(2)) + "_" + std::to_string(delta(3)) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/MN/Bootstrap/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) +  "_delta=" + std::to_string(delta(1)) + "_"
                                   + std::to_string(delta(2)) + "_" + std::to_string(delta(3))  + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/MN/Bootstrap/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_delta=" + std::to_string(delta(1)) + "_"
                                   + std::to_string(delta(2)) + "_" + std::to_string(delta(3))  + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    output.open (output_filename.c_str(), ios::out);
    size_out.open (size_filename.c_str(), ios::out);
    pwr_out.open (power_filename.c_str(), ios::out);
    ...
    //calculate 5%- and 10%- sizes of the bootstrap test
	NGPower::power_f <bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriMN>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (bootstrap_REJF, number_sampl, TL, lag_smooth,
																																											   alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s,
																																											   eta2_t, eta2_s, xi_x, xi_y, delta, rho, 0., expn, 0,
																																											   seed, size_out);
    output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << bootstrap_REJF(1) << " , " << bootstrap_REJF(2) << ")" << endl;
    ...
    //calculate 5%- and 10%- sizes of the bootstrap test
	NGPower::power_f <daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriMN>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (bootstrap_REJF, number_sampl, TL, lag_smooth,
																																											   alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s,
																																											   eta2_t, eta2_s, xi_x, xi_y, delta, rho, 0., expn, 0,
																																											   seed, size_out);
    output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << bootstrap_REJF(1) << " , " << bootstrap_REJF(2) << ")" << endl;
    ...
    //calculate 5%- and 10%- sizes of the bootstrap test
	NGPower::power_f <QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriMN>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (bootstrap_REJF, number_sampl, TL, lag_smooth,
																																											   alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s,
																																											   eta2_t, eta2_s, xi_x, xi_y, delta, rho, 0., expn, 0,
																																											   seed, size_out);
    output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << bootstrap_REJF(1) << " , " << bootstrap_REJF(2) << ")" << endl;
