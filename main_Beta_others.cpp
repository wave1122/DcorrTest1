...
int main(void) {
	...
	size_out.open (size_out_filename.c_str(), ios::out);
	auto T = 100;
	Matrix epsilon(T,1), eta1(T,1), eta2(T,1), delta(3,1);
	NL_Dgp::gen_RANV<NL_Dgp::gen_TriBeta>(epsilon, eta1, eta2, delta, 0., 0., 3, 353125235);
	for (auto t = 1; t <= T; ++t)
          size_out << epsilon(t) << " , " << eta1(t) << " , " << eta2(t) << endl;
    ...
    //*************************************************************using the Bartlett kernel****************************************************************************/
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_bartlett_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_bartlett_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_bartlett_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
	...
	//calculate empirical critical values and sizes
     Dep_tests::cValue <bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, 0., seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW <<   " are " << "(" << asymp_CV_Hong(1) << " , "
                 << asymp_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: 5%- and 10%- sizes for lag_smooth = " << bandW <<  " are "  << "(" << REJF_Hong(1) << " , " << REJF_Hong(2) << ")" << endl;
	output << "Hong's (2001) ecf-based test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW  <<  " are "  << "(" << empir_CV_ECFTest(1) << " , "
                 << empir_CV_ECFTest(2) << ")" << endl;
     output << "Hong's (2001) ecf-based test: 5%- and 10%- sizes for lag_smooth = " << bandW << " are " << "(" << REJF_ECFTest(1) << " , " << REJF_ECFTest(2) << ")" << endl;
	output << "Haugh's (1976) Portmanteau test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW << " are "  << "(" << empir_CV_Haugh(1) << " , "
                 << empir_CV_Haugh(2) << ")" << endl;
     output << "Haugh's (1976) Portmanteau test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW << " are " << "(" << asymp_CV_Haugh(1) << " , "
                 << asymp_CV_Haugh(2) << ")" << endl;
     output << "Haugh's (1976) Portmanteau test: 5%- and 10%- sizes for lag_smooth = " << bandW << " are " << "(" << REJF_Haugh(1) << " , " << REJF_Haugh(2)
                 << ")" << endl;
	//calculate rejection rates
	choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                ...
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
	...
	cout << "Haugh's (1976) Portmanteau test: asymptotic 5%- and 10%- rejection rates for choose_alt = " << choose_alt << ": " << "(" << asymp_REJF_Haugh(1) << " , "
     ...
     choose_alt = 2; //correlation = -0.005
     Dep_tests::power_f<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
	...
	cout << "Haugh's (1976) Portmanteau test: asymptotic 5%- and 10%- rejection rates for choose_alt = " << choose_alt << ": " << "(" << asymp_REJF_Haugh(1) << " , "
                                                                                                                                                                                     << asymp_REJF_Haugh(2) << ")" << endl;
     cout << "----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
     choose_alt = 3; //correlation = 0.365
     Dep_tests::power_f<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
    ...
    //***********************************************************using the Daniell kernel*************************************************************************
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_daniell_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_daniell_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_daniell_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, 0., seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW <<   " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f<daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    //******************************************************************using the QS kernel***********************************************************************
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_QS_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_QS_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_QS_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, 0., seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW <<   " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f<QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_Beta>, NL_Dgp::est_BL> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    //*************************************************************using the Bartlett kernel****************************************************************************/
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_bartlett_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_bartlett_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_bartlett_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_Beta>, NL_Dgp::est_TAR> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, 0., seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW <<   " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f<bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_Beta>, NL_Dgp::est_TAR> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    //***********************************************************using the Daniell kernel*************************************************************************
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_daniell_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_daniell_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_daniell_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_Beta>, NL_Dgp::est_TAR> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, 0., seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW <<   " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f<daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_Beta>, NL_Dgp::est_TAR> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    //******************************************************************using the QS kernel***********************************************************************
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_QS_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_QS_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(bandW) + "_QS_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     output << "Lag smooth length = " << bandW << endl;
	//calculate empirical critical values and sizes
     Dep_tests::cValue <QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_Beta>, NL_Dgp::est_TAR> (REJF_Hong, REJF_ECFTest, REJF_Haugh, empir_CV_Hong, empir_CV_ECFTest,
                                                                                                                empir_CV_Haugh, alpha_X, alpha_Y, asymp_CV_Hong, asymp_CV_Haugh, number_sampl, T, bandW, 0., seed, size_out);
     output << "Hong's (1996) spectral test: empirical 5%- and 10%- critical value for lag_smooth = " << bandW <<  " are "  << "(" << empir_CV_Hong(1) << " , "
                 << empir_CV_Hong(2) << ")" << endl;
     output << "Hong's (1996) spectral test: asymptotic 5%- and 10%- critical value for lag_smooth = " << bandW <<   " are " << "(" << asymp_CV_Hong(1) << " , "
	...
	//calculate rejection rates
	choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f<QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_Beta>, NL_Dgp::est_TAR> (empir_REJF_Hong, asymp_REJF_Hong, empir_REJF_ECFTest, asymp_REJF_ECFTest,
                                                                                                                                                                                empir_REJF_Haugh, asymp_REJF_Haugh, alpha_X, alpha_Y, empir_CV_Hong,
                                                                                                                                                                                asymp_CV_Hong, empir_CV_ECFTest, asymp_CV_Hong, empir_CV_Haugh,
                                                                                                                                                                                asymp_CV_Haugh, number_sampl, T, bandW, 0., 0., choose_alt, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
    ...
    /************************************************************************************************************************************************/
     ...
     bandW = 5;
     output << "Lag smooth length = " << bandW << endl;
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_BL_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_HRTest.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_BL_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_HRTest.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_BL_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_HRTest.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     //calculate empirical critical values and sizes
	Matrix asymp_CV_ElHimdiRoy = Dep_tests::asymp_CV_ChiSq2 (bandW); //obtain 5%- and 10%- asymptotic critical values for the test
     Dep_tests::cValue <NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriBeta>, NL_Dgp::est_BL> (REJF_ElHimdiRoy, empir_CV_ElHimdiRoy, alpha_X, alpha_Y, delta, asymp_CV_ElHimdiRoy,
     ...
     //calculate rejection rates
     choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f <NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriBeta>, NL_Dgp::est_BL> (empir_REJF_ElHimdiRoy, asymp_REJF_ElHimdiRoy, alpha_X, alpha_Y, delta,
    ...
    /************************************************************************************************************************************************/
     ...
     bandW = 5;
     output << "Lag smooth length = " << bandW << endl;
     output_filename = "./Results/NonGaussian/BETA/Others/file_out_TAR_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_HRTest.txt";
	size_filename = "./Results/NonGaussian/BETA/Others/size_out_TAR_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_HRTest.txt";
     power_filename = "./Results/NonGaussian/BETA/Others/power_out_TAR_T=" + std::to_string(T) +  "_bandW=" + std::to_string(bandW) + "_HRTest.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     //calculate empirical critical values and sizes
	Matrix asymp_CV_ElHimdiRoy = Dep_tests::asymp_CV_ChiSq2 (bandW); //obtain 5%- and 10%- asymptotic critical values for the test
     Dep_tests::cValue <NL_Dgp::gen_TAR<NL_Dgp::gen_TriBeta>, NL_Dgp::est_TAR> (REJF_ElHimdiRoy, empir_CV_ElHimdiRoy, alpha_X, alpha_Y, delta, asymp_CV_ElHimdiRoy,
                                                                                                                                                          number_sampl, T, bandW, seed, size_out);
     output << "5%- and 10%- empirical critical values for bandW = " <<  bandW << ":  " << "(" << empir_CV_ElHimdiRoy(1) << " , " << empir_CV_ElHimdiRoy(2) << ")" << endl;
     output << "5%- and 10%- sizes for bandW = " <<  bandW << ":  " << "(" << REJF_ElHimdiRoy(1) << " , " << REJF_ElHimdiRoy(2) << ")" << endl;
     cout << "5%- and 10%- empirical critical values for bandW = " <<  bandW << ":  " << "(" << empir_CV_ElHimdiRoy(1) << " , " << empir_CV_ElHimdiRoy(2) << ")" << endl;
     ...
     //calculate rejection rates
     choose_alt = 1;  //correlation = -0.145
     Dep_tests::power_f <NL_Dgp::gen_TAR<NL_Dgp::gen_TriBeta>, NL_Dgp::est_TAR> (empir_REJF_ElHimdiRoy, asymp_REJF_ElHimdiRoy, alpha_X, alpha_Y, delta,
                                                                                                                                                           empir_CV_ElHimdiRoy, asymp_CV_ElHimdiRoy, 0., 0., choose_alt, number_sampl,
                                                                                                                                                           T, bandW, seed, pwr_out);
     output << "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
     output << "choose_alt = " << choose_alt << endl;
