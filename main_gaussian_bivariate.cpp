...
int main(void) {
    ...
    auto T = 60, TL = 4, lag_smooth = 5;
    Matrix X(T, 1), Y1(T, 1), Y2(T, 1), resid(T-2, 1), slope(6, 1), alpha_X(6, 1), alpha_Y(6, 1);
    alpha_X(1) = 0.01;
    alpha_X(2) = 0.5;
    alpha_X(3) = -0.2;
    alpha_X(4) = -0.1;
    ...
    //NGReg::var_U_ts <NL_Dgp::gen_MN, NGReg::cmean_TAR> (&res_X, &res_Y, xy_t, xy_s, lag_t1, lag_t2, lag_s1, lag_s2, alpha_X, alpha_Y,0.7, -0.5, 1.5, r, 1342525);
    //NGReg::var_U_ts <NL_Dgp::gen_TriMN, NGReg::cmean_BL> (&res_X, &res_Y, xy_t, xy_s, lag_t1, lag_t2, lag_s1, lag_s2, alpha_X, alpha_Y, delta, rho12, rho13, 1.5, r, 1352523);
	//cout << res_X << " , " << res_Y << endl;
	//generate data
	//Matrix data_X(T,1), data_Y1(T,1), data_Y2(T,1);
    //NL_Dgp::gen_TAR<NL_Dgp::gen_TriSN> (data_X, data_Y1, data_Y2, alpha_X, alpha_Y, delta, rho12, rho13, 1, 1345253);
    //auto kernel_QDSum = 0., kernel_QRSum = 0.;
    //NGDist_corr::integrate_Kernel <daniell_kernel> (kernel_QDSum, kernel_QRSum);
	//cout << NGDist_corr::do_Test<daniell_kernel, NL_Dgp::gen_TriSN, NGReg::cmean_TAR> (data_X, data_Y1, data_Y2, TL, lag_smooth, kernel_QRSum, alpha_X,
	//                                                                                   alpha_Y, delta, 1.5, 1245355) << endl;
	Matrix asymp_REJF(2,1), empir_CV(2,1), asymp_CV(2,1);
    ...
    NGPower::cValue<daniell_kernel,NL_Dgp::gen_SN,NL_Dgp::gen_Bilinear<NL_Dgp::gen_SN>,NGReg::cmean_BL, NL_Dgp::est_BL> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                       alpha_X, alpha_Y, delta(1), expn, asymp_CV, 1234235, size_out);
    size_out.close();
    /*Matrix X(500, 1), Y1(500, 1), Y2(500, 1), resid(500-2,1), slope(5,1), alpha_X(5, 1), alpha_Y(5, 2), delta(3,1);
    alpha_X(1) = 0.01;
    alpha_X(2) = 0.7;
    alpha_X(3) = 0.1;
    alpha_X(4) = -0.6;
    ...
    gsl_rng *r = nullptr;
	int choose_alt = 2;
    NL_Dgp::gen_TAR<NL_Dgp::gen_TriSN>(X, Y1, Y2, alpha_X, alpha_Y, delta, rho12, rho13, choose_alt, r, 341242);
    NL_Dgp::est_TAR (resid, slope, Y2);
    ofstream output;
	output.open ("./test_output.txt", ios::out);
    for (int t = 1; t <= X.nRow(); t++)
        output << X(t) << " , " << Y1(t) << " , " << Y2(t) << endl;
    output << "----------------------------------------------------------------------\n";
	...
	*/
    ...
    #if 0 //begin comment
    /**************************************************** Start the Simulations for the Proposed Test: The Univariate Case **************************************/
    /************************************************************************************************************************************************************/
	int TL = 2;
    int T = 200;
	int number_sampl = 500;
