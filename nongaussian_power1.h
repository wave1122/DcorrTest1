...
void NGPower::cValue (Matrix &asymp_REJF, Matrix &empir_CV, const int number_sampl, const int T, const int TL, const int lag_smooth, const Matrix &alpha_X,
	...
	#pragma omp parallel for default(shared) reduction (+:asymp_REJ_5,asymp_REJ_10) schedule(dynamic,CHUNK) private(i) firstprivate(rseed,X,Y,resid_X,resid_Y,alpha_X_hat,alpha_Y_hat)
...
void NGPower::cValue (Matrix &asymp_REJF, Matrix &empir_CV, const int number_sampl, const int T, const int TL, const int lag_smooth, const Matrix &alpha_X,
    ...
    int asymp_REJ_5 = 0, asymp_REJ_10 = 0;
	#pragma omp parallel for default(shared) reduction (+:asymp_REJ_5,asymp_REJ_10) schedule(dynamic,CHUNK) private(i,j) firstprivate(rseed,X,Y1,Y2,resid_X,resid_Y1,resid_Y2,alpha_X_hat,alpha_Y1_hat,alpha_Y2_hat,alpha_Y_hat)
	for (i = 1; i <= number_sampl; i++) {
        rseed = gsl_rng_get (r);
...
void NGPower::power_f (Matrix &empir_REJF, Matrix &asymp_REJF, const int number_sampl, const int T, const int TL, const int lag_smooth, const Matrix &alpha_X,
	...
	#pragma omp parallel for default(shared) reduction (+:empir_REJ_5,empir_REJ_10,asymp_REJ_5,asymp_REJ_10) schedule(dynamic,CHUNK) private(i) firstprivate(rseed,X,Y,resid_X,resid_Y,alpha_X_hat,alpha_Y_hat,tvalue)
	for (i = 1; i <= number_sampl; ++i) {
...
void NGPower::power_f (Matrix &empir_REJF, Matrix &asymp_REJF, const int number_sampl, const int T, const int TL, const int lag_smooth, const Matrix &alpha_X,
	...
	#pragma omp parallel for default(shared) reduction (+:empir_REJ_5,empir_REJ_10,asymp_REJ_5,asymp_REJ_10) schedule(dynamic,CHUNK) private(i,j) firstprivate(rseed,X,Y1,Y2,resid_X,resid_Y1,resid_Y2,alpha_X_hat,alpha_Y1_hat,alpha_Y2_hat,alpha_Y_hat,tvalue)
	for (i = 1; i <= number_sampl; ++i) {
		rseed = gsl_rng_get (r); //set a random seed for the random number generator
		//draw two samples from the dgp of X and Y each using a random seed
		...
		//calculate the test statistic
		tvalue = NGDist_corr::do_Test<kernel, cmean> (X, Y1, Y2, TL, lag_smooth, kernel_QRSum, alpha_X_hat, alpha_Y_hat, epsilon_t, epsilon_s, eta1_t,
		                                              eta1_s, eta2_t, eta2_s, expn);
        pwr_out << tvalue << endl;
        if (tvalue >= empir_CV(1)) ++empir_REJ_5;//using 5%-critical value
        if (tvalue >= asymp_CV(1)) ++asymp_REJ_5;//using 5%-critical value
        if (tvalue >= empir_CV(2)) ++empir_REJ_10;//using 10%-critical value
