...
int main(void) {
	...
	*/
    ...
    alpha_Y(5) = 0.1;
    alpha_Y(6) = 0.1;
    NL_Dgp::gen_RANV<NL_Dgp::gen_ChiSq> (epsilon_t, eta_t, 0., 0., 0, 13435); //generate independent Beta random errors
    NL_Dgp::gen_RANV<NL_Dgp::gen_ChiSq> (epsilon_s, eta_s, 0., 0., 0, 53435);

    /************************************************************ Using the Bartlett kernel ***********************************************************************/
     output_filename = "./Results/NonGaussian/CHISQ/file_out_BL_T=" + std::to_string(T) +  "_N= "  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) +  "_expn=" + std::to_string(expn) + "_bartlett_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/CHISQ/size_out_BL_T=" + std::to_string(T)  +  "_N= "  + std::to_string(N)  +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/CHISQ/power_out_BL_T=" + std::to_string(T)  +  "_N= "  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     ...
     NGPower::cValue<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_ChiSq>, NGReg::cmean_BL, NL_Dgp::est_BL> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                            alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., expn, asymp_CV, seed, size_out);
     ...
     NGPower::power_f<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_ChiSq>, NGReg::cmean_BL, NL_Dgp::est_BL> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                               alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., rho, expn, empir_CV, asymp_CV, choose_alt, seed, pwr_out);
    ...
    /************************************************************ Using the Daniell kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/CHISQ/file_out_BL_T=" + std::to_string(T) +  "_N= "  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) +  "_expn=" + std::to_string(expn) + "_daniell_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/CHISQ/size_out_BL_T=" + std::to_string(T)  +  "_N= "  + std::to_string(N)  +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_daniell_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/CHISQ/power_out_BL_T=" + std::to_string(T)  +  "_N= "  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_daniell_kernel_univariate.txt";
     ...
     output << "Lag smooth length = " << lag_smooth << endl;
     //calculate 5%- and 10%- asymptotic sizes and empirical critical values
     NGPower::cValue<daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_ChiSq>, NGReg::cmean_BL, NL_Dgp::est_BL> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                            alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., expn, asymp_CV, seed, size_out);
     output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     choose_alt = 1; //correlated errors
     NGPower::power_f<daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_ChiSq>, NGReg::cmean_BL, NL_Dgp::est_BL> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                               alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., rho, expn, empir_CV, asymp_CV, choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt <<" and rho = " << rho << ": (" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	output << "5%- and 10% - asymptotic rejection rates for choose_alt = " << choose_alt  <<" and rho = " << rho << ": ("  << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt  <<" and rho = " << rho << ": ("  << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
    ...
    /************************************************************ Using the QS kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/CHISQ/file_out_BL_T=" + std::to_string(T) +  "_N= "  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) +  "_expn=" + std::to_string(expn) + "_QS_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/CHISQ/size_out_BL_T=" + std::to_string(T)  +  "_N= "  + std::to_string(N)  +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_QS_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/CHISQ/power_out_BL_T=" + std::to_string(T)  +  "_N= "  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_QS_kernel_univariate.txt";
     ...
     output << "Lag smooth length = " << lag_smooth << endl;
     //calculate 5%- and 10%- asymptotic sizes and empirical critical values
     NGPower::cValue<QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_ChiSq>, NGReg::cmean_BL, NL_Dgp::est_BL> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                            alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., expn, asymp_CV, seed, size_out);
     output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     choose_alt = 1; //correlated errors
     NGPower::power_f<QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_ChiSq>, NGReg::cmean_BL, NL_Dgp::est_BL> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                               alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., rho, expn, empir_CV, asymp_CV, choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt <<" and rho = " << rho << ": (" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	output << "5%- and 10% - asymptotic rejection rates for choose_alt = " << choose_alt  <<" and rho = " << rho << ": ("  << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt  <<" and rho = " << rho << ": ("  << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
    ...
    /************************************************************ Using the Bartlett kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/CHISQ/file_out_TAR_T=" + std::to_string(T) +  "_N="  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) +  "_expn=" + std::to_string(expn) + "_bartlett_kernel_univariate.txt";
	size_filename = "./Results/NonGaussian/CHISQ/size_out_TAR_T=" + std::to_string(T)  +  "_N="  + std::to_string(N)  +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_univariate.txt";
     power_filename = "./Results/NonGaussian/CHISQ/power_out_TAR_T=" + std::to_string(T)  +  "_N="  + std::to_string(N) + "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_rho=" + std::to_string(rho) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_univariate.txt";
     output.open (output_filename.c_str(), ios::out);
     size_out.open (size_filename.c_str(), ios::out);
     pwr_out.open (power_filename.c_str(), ios::out);
     output << "Lag smooth length = " << lag_smooth << endl;
     //calculate 5%- and 10%- asymptotic sizes and empirical critical values
     NGPower::cValue<bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_ChiSq>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                            alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., expn, asymp_CV, seed, size_out);
     output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     choose_alt = 1; //correlated errors
     NGPower::power_f<bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_ChiSq>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                               alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., rho, expn, empir_CV, asymp_CV, choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt <<" and rho = " << rho << ": (" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	output << "5%- and 10% - asymptotic rejection rates for choose_alt = " << choose_alt  <<" and rho = " << rho << ": ("  << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt  <<" and rho = " << rho << ": ("  << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	...
	size_filename = "./Results/NonGaussian/CHISQ/size_out_TAR_T=" + std::to_string(T)  +  "_N="  + std::to_string(N)  +  "_lag_smooth=" + std::to_string(lag_smooth)
     ...
     output << "Lag smooth length = " << lag_smooth << endl;
     //calculate 5%- and 10%- asymptotic sizes and empirical critical values
     NGPower::cValue<daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_ChiSq>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                            alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., expn, asymp_CV, seed, size_out);
     output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     choose_alt = 1; //correlated errors
     NGPower::power_f<daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_ChiSq>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                               alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., rho, expn, empir_CV, asymp_CV, choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt <<" and rho = " << rho << ": (" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	output << "5%- and 10% - asymptotic rejection rates for choose_alt = " << choose_alt  <<" and rho = " << rho << ": ("  << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt  <<" and rho = " << rho << ": ("  << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	...
	size_filename = "./Results/NonGaussian/CHISQ/size_out_TAR_T=" + std::to_string(T)  +  "_N="  + std::to_string(N)  +  "_lag_smooth=" + std::to_string(lag_smooth)
     ...
     output << "Lag smooth length = " << lag_smooth << endl;
     //calculate 5%- and 10%- asymptotic sizes and empirical critical values
     NGPower::cValue<QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_ChiSq>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                            alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., expn, asymp_CV, seed, size_out);
     output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
     output << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     choose_alt = 1; //correlated errors
     NGPower::power_f<QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_ChiSq>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                               alpha_X, alpha_Y, epsilon_t, epsilon_s, eta_t, eta_s, 0., rho, expn, empir_CV, asymp_CV, choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt <<" and rho = " << rho << ": (" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	output << "5%- and 10% - asymptotic rejection rates for choose_alt = " << choose_alt  <<" and rho = " << rho << ": ("  << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt  <<" and rho = " << rho << ": ("  << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
