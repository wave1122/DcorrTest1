...
int main(void) {
	...
	*/
	#if 0 //begin comment
	/******************************************* Start the Simulations for the Proposed Test: The Bivariate Bilinear Case ***************************************/
    /************************************************************************************************************************************************************/
     int TL = 2;
     int T = 100;
     int N = 500;
	...
	int choose_alt = 0;
     ...
     Matrix asymp_CV(2,1), empir_REJF(2,1), empir_CV(2,1), asymp_REJF(2,1), alpha_X(6,1), alpha_Y(6,2), epsilon_t(N,1), epsilon_s(N,1), eta1_t(N,1), eta1_s(N,1), eta2_t(N,1),
     ...
     NL_Dgp::gen_RANV<NL_Dgp::gen_TriExp> (epsilon_t, eta1_t, eta2_t, delta, 0., 0., 0, 13435); //generate independent exponential random errors
     NL_Dgp::gen_RANV<NL_Dgp::gen_TriExp> (epsilon_s, eta1_s, eta2_s, delta, 0., 0., 0, 53435);

    /************************************************************ Using the Bartlett kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/EXP/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/EXP/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/EXP/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    output.open (output_filename.c_str(), ios::out);
    size_out.open (size_filename.c_str(), ios::out);
    pwr_out.open (power_filename.c_str(), ios::out);
    output << "Lag smooth length = " << lag_smooth << endl;
    //calculate 5%- and 10%- asymptotic sizes and empirical critical values
     NGPower::cValue<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriExp>, NGReg::cmean_BL, NL_Dgp::est_BL> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                     alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                     eta2_s, delta, expn, asymp_CV, seed, size_out);
    ...
    NGPower::power_f<bartlett_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriExp>, NGReg::cmean_BL, NL_Dgp::est_BL> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                       alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                       eta2_s, delta, 0., 0., expn, empir_CV, asymp_CV,
                                                                                                                                                                                                                       choose_alt, seed, pwr_out);
    ...
    /************************************************************ Using the Daniell kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/EXP/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_daniell_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/EXP/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_daniell_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/EXP/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_daniell_kernel_bivariate.txt";
    output.open (output_filename.c_str(), ios::out);
    size_out.open (size_filename.c_str(), ios::out);
    pwr_out.open (power_filename.c_str(), ios::out);
    output << "Lag smooth length = " << lag_smooth << endl;
    //calculate 5%- and 10%- asymptotic sizes and empirical critical values
    NGPower::cValue<daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriExp>, NGReg::cmean_BL, NL_Dgp::est_BL> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                     alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                     eta2_s, delta, expn, asymp_CV, seed, size_out);
    output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    ...
    cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
    choose_alt = 1; //errors are very weakly correlated
    NGPower::power_f<daniell_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriExp>, NGReg::cmean_BL, NL_Dgp::est_BL> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                       alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                       eta2_s, delta, 0., 0., expn, empir_CV, asymp_CV,
                                                                                                                                                                                                                       choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt << ": " << "(" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
    ...
    /************************************************************ Using the QS kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/EXP/file_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_QS_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/EXP/size_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_QS_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/EXP/power_out_BL_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth) + "_expn=" + std::to_string(expn) + "_QS_kernel_bivariate.txt";
    output.open (output_filename.c_str(), ios::out);
    size_out.open (size_filename.c_str(), ios::out);
    pwr_out.open (power_filename.c_str(), ios::out);
    output << "Lag smooth length = " << lag_smooth << endl;
    //calculate 5%- and 10%- asymptotic sizes and empirical critical values
    NGPower::cValue<QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriExp>, NGReg::cmean_BL, NL_Dgp::est_BL> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                     alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                     eta2_s, delta, expn, asymp_CV, seed, size_out);
    output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    ...
    cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
    choose_alt = 1; //errors are very weakly correlated
    NGPower::power_f<QS_kernel, NL_Dgp::gen_Bilinear<NL_Dgp::gen_TriExp>, NGReg::cmean_BL, NL_Dgp::est_BL> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                       alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                       eta2_s, delta, 0., 0., expn, empir_CV, asymp_CV,
                                                                                                                                                                                                                       choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt << ": " << "(" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	...
	output.close();
     size_out.close();
     pwr_out.close();
	/***************************************** Finish the Simulations for the Proposed Test: The Bivariate Bilinear Case*****************************************/
     /************************************************************************************************************************************************************/
	#endif //end comment
	...
	//#if 0 //begin comment
	/******************************************* Start the Simulations for the Proposed Test: The Bivariate TAR Case ***************************************/
    /************************************************************************************************************************************************************/
     int TL = 2;
     int T = 100;
     int N = 500;
    ...
    /************************************************************ Using the Bartlett kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/EXP/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/EXP/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/EXP/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_bartlett_kernel_bivariate.txt";
    output.open (output_filename.c_str(), ios::out);
    size_out.open (size_filename.c_str(), ios::out);
    pwr_out.open (power_filename.c_str(), ios::out);
    output << "Lag smooth length = " << lag_smooth << endl;
    //calculate 5%- and 10%- asymptotic sizes and empirical critical values
     NGPower::cValue<bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriExp>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                     alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                     eta2_s, delta, expn, asymp_CV, seed, size_out);
    output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    ...
    cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
    choose_alt = 1; //errors are very weakly correlated
    NGPower::power_f<bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriExp>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                       alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                       eta2_s, delta, 0., 0., expn, empir_CV, asymp_CV,
                                                                                                                                                                                                                       choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt << ": " << "(" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	output << "5%- and 10% - asymptotic rejection rates for choose_alt = " << choose_alt << ": " << "(" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
     cout << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt << ": " << "(" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
	cout << "5%- and 10% - asymptotic rejection rates for choose_alt = " <<  choose_alt << ": " << "(" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
    choose_alt = 2; //errors are weakly correlated
	NGPower::power_f<bartlett_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriExp>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                        alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                        eta2_s, delta, 0., 0., expn, empir_CV, asymp_CV,
                                                                                                                                                                                                                        choose_alt, seed, pwr_out);
    ...
    /************************************************************ Using the Daniell kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/EXP/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_daniell_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/EXP/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_daniell_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/EXP/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_daniell_kernel_bivariate.txt";
    ...
    output << "Lag smooth length = " << lag_smooth << endl;
    //calculate 5%- and 10%- asymptotic sizes and empirical critical values
    NGPower::cValue<daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriExp>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                     alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                     eta2_s, delta, expn, asymp_CV, seed, size_out);
    output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    ...
    cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
    choose_alt = 1; //errors are very weakly correlated
    NGPower::power_f<daniell_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriExp>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                       alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                       eta2_s, delta, 0., 0., expn, empir_CV, asymp_CV,
                                                                                                                                                                                                                       choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt << ": " << "(" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
    ...
    /************************************************************ Using the QS kernel ***********************************************************************/
    output_filename = "./Results/NonGaussian/EXP/file_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_QS_kernel_bivariate.txt";
	size_filename = "./Results/NonGaussian/EXP/size_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_QS_kernel_bivariate.txt";
    power_filename = "./Results/NonGaussian/EXP/power_out_TAR_T=" + std::to_string(T) +  "_lag_smooth=" + std::to_string(lag_smooth)
                                   + "_expn=" + std::to_string(expn) + "_QS_kernel_bivariate.txt";
    ...
    output << "Lag smooth length = " << lag_smooth << endl;
    //calculate 5%- and 10%- asymptotic sizes and empirical critical values
    NGPower::cValue<QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriExp>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (asymp_REJF, empir_CV, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                     alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                     eta2_s, delta, expn, asymp_CV, seed, size_out);
    output << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    cout << "5%- and 10%- empirical critical values for lag_smooth = " << lag_smooth << ":  " << "(" << empir_CV(1) << " , " << empir_CV(2) << ")" << endl;
    ...
    cout << "5% - and 10% - sizes for the lag_smooth = " << lag_smooth << ": (" << asymp_REJF(1) << " , " << asymp_REJF(2) << ")" << endl;
    choose_alt = 1; //errors are very weakly correlated
    NGPower::power_f<QS_kernel, NL_Dgp::gen_TAR<NL_Dgp::gen_TriExp>, NGReg::cmean_TAR, NL_Dgp::est_TAR> (empir_REJF, asymp_REJF, number_sampl, T, TL, lag_smooth,
                                                                                                                                                                                                                       alpha_X, alpha_Y, epsilon_t, epsilon_s, eta1_t, eta1_s, eta2_t,
                                                                                                                                                                                                                       eta2_s, delta, 0., 0., expn, empir_CV, asymp_CV,
                                                                                                                                                                                                                       choose_alt, seed, pwr_out);
	output << "5%- and 10% - empirical rejection rates for choose_alt = " <<  choose_alt << ": " << "(" << empir_REJF(1) << " , " << empir_REJF(2) << ")" << endl;
    ...
    size_out.close();
    pwr_out.close();
	/***************************************** Finish the Simulations for the Proposed Test: The Bivariate TAR Case *********************************************************/
    /************************************************************************************************************************************************************/
	//#endif //end comment
